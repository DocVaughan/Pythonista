...
class MyScene (Scene):
	def setup(self):
		...
		self.HOST, self.PORT = '192.168.0.102', 2390
	...
	def draw(self):
		...
		self.sock.sendto(data_st_r.encode('utf-8'), (self.HOST, self.PORT))

		# Show the output on the screen
		text('Output = ' + data_str, font_name='Helvetica', font_size=24.0, x=self.size.w/2., y=50., alignment=5)
	...
	def touch_began(self, touch):
		...
		self.range = Layer(Rect(self.initial.x - 100.0, self.initial.y - 100, 200, 200))
		#self.range.background = Color(0.0, 0.5, 0.0)
		self.range.image = 'Black_Square'
		self.range.alpha = 0.6
		self.root_layer.add_layer(self.range)


		# Layer for the joystick "bubble"
		self.layer = Layer(Rect(self.current.x - 64, self.current.y - 64, 128, 128))
...
#			# Set up a new frame based on latest touch location
		...
		#range_frame = Rect(self.initial.x - 100.0, self.initial.y - 100, 200, 200)
	...
	def touch_moved(self, touch):
		...
		#deltaX = (self.current.x - self.initial.x)/(self.size.w/2.-64)*100
		#deltaY = (self.current.y - self.initial.y)/(self.size.h/2.-64)*100

		deltaX = (self.current.x - self.initial.x)/100.0*100
		deltaY = (self.current.y - self.initial.y)/100.0*100
	...
	def touch_ended(self, touch):
		...
		self.root_layer.remove_layer(self.range)
		self.root_layer.remove_layer(self.layer)
